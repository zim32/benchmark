...
const Rectangle = struct {
    ...
    fn compute_area(self: *const Rectangle) f64 {
        const a = @as(f64, @floatFromInt(self.size_a));
        const b = @as(f64, @floatFromInt(self.size_b));
        return a * b;
    }
};
...
const Triangle = struct {
    ...
    fn compute_area(self: *const Triangle) f64 {
        const a = @as(f64, @floatFromInt(self.size_a));
        const b = @as(f64, @floatFromInt(self.size_b));
        const c = @as(f64, @floatFromInt(self.size_c));
        const s = (a + b + c) / 2.0;
...
const Circle = struct {
    ...
    fn compute_area(self: *const Circle) f64 {
        const d = @as(f64, @floatFromInt(self.diameter));
        const r: f64 = d / 2.0;
        return math.pi * math.pow(f64, r, 2.0);
    }
};

const FigureTag = enum { square, rectangle, triangle, circle };

const Figure = union(FigureTag) {
    square: Square,
    rectangle: Rectangle,
    triangle: Triangle,
    circle: Circle,
...
fn compute_figure_area(figure: *const Figure) f64 {
    return switch (figure.*) {
        FigureTag.square => |item| item.compute_area(),
        FigureTag.rectangle => |item| item.compute_area(),
        FigureTag.triangle => |item| item.compute_area(),
        FigureTag.circle => |item| item.compute_area(),
    };
}

fn get_figure_type(figure: *const Figure) []const u8 {
    return switch (figure.*) {
        FigureTag.square => "square",
        FigureTag.rectangle => "rectangle",
        FigureTag.triangle => "triangle",
        FigureTag.circle => "circle",
    };
}

fn load_figures(allocator: std.mem.Allocator) []Figure {
    var parsed = std.json.parseFromSlice(std.json.Value, allocator, DATA, .{}) catch return undefined;
    defer parsed.deinit();

    var result: []Figure = allocator.alloc(Figure, 500) catch return undefined;

    for (parsed.value.array.items, 0..) |item, index| {
        const figure_type = item.object.get("type").?.string;
        var figure: Figure = undefined;

        if (std.mem.eql(u8, figure_type, "square")) {
            figure = Figure{ .square = Square{ .size = item.object.get("size").?.integer } };
        }
        if (std.mem.eql(u8, figure_type, "rectangle")) {
            figure = Figure{ .rectangle = Rectangle{
                .size_a = item.object.get("sizeA").?.integer,
                .size_b = item.object.get("sizeB").?.integer,
            } };
        }
        if (std.mem.eql(u8, figure_type, "triangle")) {
            figure = Figure{ .triangle = Triangle{
                .size_a = item.object.get("sizeA").?.integer,
                .size_b = item.object.get("sizeB").?.integer,
                .size_c = item.object.get("sizeC").?.integer,
            } };
        }
        if (std.mem.eql(u8, figure_type, "circle")) {
            figure = Figure{ .circle = Circle{ .diameter = item.object.get("diameter").?.integer } };
        }
...
fn on_get_request(r: zap.SimpleRequest) void {
    ...
    var min_area: f64 = math.floatMax(f64);
    var max_area: f64 = math.floatMin(f64);
    var total_area: f64 = @as(f64, 0.0);

    var figure_types = std.StringArrayHashMap(std.json.Value).init(allocator.allocator());
    figure_types.put("square", std.json.Value{ .integer = 0 }) catch return;
    figure_types.put("rectangle", std.json.Value{ .integer = 0 }) catch return;
    figure_types.put("triangle", std.json.Value{ .integer = 0 }) catch return;
    figure_types.put("circle", std.json.Value{ .integer = 0 }) catch return;

    const figures = load_figures(allocator.allocator());

    for (figures) |figure| {
        const area = compute_figure_area(&figure);
        min_area = min_value(min_area, area);
        max_area = max_value(max_area, area);
        total_area += area;

        const figure_type = get_figure_type(&figure);
        const cnt = figure_types.get(figure_type).?.integer;

        figure_types.put(figure_type, std.json.Value{ .integer = cnt + 1 }) catch return;
    }

    var figure_types_json = std.json.Value{ .object = figure_types };
    ...
    var response = .{
        ...
        .figure_types = figure_types_json,
    };
    ...
    var response_json = std.ArrayList(u8).init(fba.allocator());
    std.json.stringify(response, .{}, response_json.writer()) catch return;
    var trimmed_buff = std.mem.trim(u8, &buf, &[_]u8{0});

    // send response
    r.sendBody(trimmed_buff) catch return;
}

fn on_post_request(r: zap.SimpleRequest) void {
    var allocator = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer allocator.deinit();
    ...
    var response = .{
        .app = "zig-zap***",
        .time = std.time.milliTimestamp() - start,
        .min_area = min_area,
        .max_area = max_area,
        .total_area = total_area,
    };
