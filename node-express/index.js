...
function computeRectangleArea(sizeA, sizeB) {
    return sizeA * sizeB
}

function computeTriangleArea(sizeA, sizeB, sizeC) {
    const s = (sizeA + sizeB + sizeC) / 2
    return Math.sqrt(s * (s - sizeA) * (s - sizeB) * (s - sizeC))
}

function computeCircleArea(diameter) {
    const r = diameter / 2
    return Math.PI * Math.pow(r, 2)
}

function computeFigureArea(figure) {
    switch (figure['type']) {
        case 'square':
            return computeSquareArea(figure['size'])
        case 'rectangle':
            return computeRectangleArea(figure['sizeA'], figure['sizeB'])
        case 'triangle':
            return computeTriangleArea(figure['sizeA'], figure['sizeB'], figure['sizeC'])
        case 'circle':
            return computeCircleArea(figure['diameter'])
    }
}
