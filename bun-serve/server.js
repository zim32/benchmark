...
class Triangle {
    ...
    computeArea() {
        const s = (this.sizeA + this.sizeB + this.sizeC) / 2
        return Math.sqrt(s * (s - this.sizeA) * (s - this.sizeB) * (s - this.sizeC))
    }
...
function loadFigures() {
    ...
    return json.map(item => {
        switch (item.type) {
            case 'square':
                return new Square(item.size)
            case 'rectangle':
                return new Rectangle(item.sizeA, item.sizeB)
            case 'triangle':
                return new Triangle(item.sizeA, item.sizeB, item.sizeC)
            case 'circle':
                return new Circle(item.diameter)
        }
    })
}
...
serve({
  ...
  async fetch(req) {
    ...
    if (url.pathname === '/ping-v2' && method === 'GET') {
        return handleGetPingV2()
    }

    if (url.pathname === '/ping' && method === 'POST') {
        return handlePostPing()
    }
