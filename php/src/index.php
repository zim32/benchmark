...
use Slim\App;
use Zim32\Php\Circle;
use Zim32\Php\FigureInterface;
use Zim32\Php\Rectangle;
use Zim32\Php\Square;
use Zim32\Php\Triangle;
 ...
 */
function loadFigures(): array
{
    global $data;

    $decodedData = json_decode($data, 1);
    /** @var FigureInterface[] $figures */
    $figures = [];

    foreach ($decodedData as $json) {
        $figure = match ($json['type']) {
            'square' => new Square('square', $json['size']),
            'rectangle' => new Rectangle('rectangle', $json['sizeA'], $json['sizeB']),
            'triangle' => new Triangle('triangle', $json['sizeA'], $json['sizeB'], $json['sizeC']),
            'circle' => new Circle('circle', $json['diameter']),
        };
...
$app->get('/ping', function(RequestInterface $request, ResponseInterface $response) {
    ...
    $totalArea = 0.0;
    $figureTypes = ['square' => 0, 'rectangle' => 0, 'triangle' => 0, 'circle' => 0];
...
$server->on("request", function (Request $request, Response $response) use ($app, $serverRequestFactory) {
    $psr7Request = $serverRequestFactory->createFromSwoole($request);
    $psr7Response = $app->handle($psr7Request);
    $swooleResponseConverter = new SwooleResponseConverter($response);
    $swooleResponseConverter->send($psr7Response);
});
