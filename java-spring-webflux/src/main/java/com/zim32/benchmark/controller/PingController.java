package com.zim32.benchmark.controller;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.zim32.benchmark.model.FigureInterface;
...
class GetResponse {
    ...
    @JsonProperty("figure_types")
    public HashMap<String, Integer> figureTypes;
    ...
    GetResponse() {
        ...
        totalArea = 0.0;
        figureTypes = new HashMap<>(Map.of(
                "square", 0,
                "rectangle", 0,
                "triangle", 0,
                "circle", 0
...
""";
    ...
    @GetMapping("/ping")
    public Mono<GetResponse> getPing() {
        var figures = loadFigures();
        var response = new GetResponse();

        for (var figure: figures) {
            var area = figure.computeArea();
            response.minArea = Math.min(response.minArea, area);
            response.maxArea = Math.max(response.maxArea, area);
            response.totalArea += area;

            var type = figure.getType();
            var cnt = response.figureTypes.get(type);
            response.figureTypes.put(type, cnt + 1);
        }
    ...
    @PostMapping("/ping")
    public Mono<PostResponse> postPing() {
        var figures = loadFigures();
    ...
    private FigureInterface[] loadFigures() {
        var objectMapper = new ObjectMapper();

        try {
            return objectMapper.readValue(DATA, FigureInterface[].class);
